...
namespace warehub.db
{
    ...
    public class CRUDService : ICRUDService
    {
        ...
        public async Task<bool> Create(string table, Dictionary<string, object> parameters)
        {
            try
            {
                ...
                if (await _queryExecutor.ExecuteNonQuery(query, parameters, $"Item created in table '{table}' with values: {string.Join(", ", parameters.Select(p => $"{p.Key}={p.Value}"))}."))
                {
        ...
        public async Task<(bool, List<Dictionary<string, object>>)> Read(string table, Dictionary<string, object> parameters)
        {
            try
            {
                ...
                var (status, results) = await _queryExecutor.ExecuteQuery(query, parameters, $"Data retrieved from table '{table}'.", columnTypeMapping);
        ...
        /// <returns>True if the operation is successful; otherwise, false.</returns>
        public async Task<bool> Update(string table, Dictionary<string, object> parameters, string idColumn, object idValue)
        {
            try
            {
                ...
                if (await _queryExecutor.ExecuteNonQuery(query, parameters, $"Item updated successfully in table '{table}' with {idColumn}={idValue}."))
                {
        ...
        /// <param name="idValue">The value of the identifying column.</param>
        /// <returns>True if the operation is successful; otherwise, false.</returns>
        public async Task<bool> Delete(string table, string idColumn, object idValue)
        {
            try
            {
                ...
                if (await _queryExecutor.ExecuteNonQuery(query, parameters, $"Item deleted successfully in table '{table}' with {idColumn}={idValue}."))
                {
